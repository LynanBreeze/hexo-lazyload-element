...
const getformatedStr = (str, attrs, placeholerStyle = '') => {
  ...
  if (style) {
    formatedStr = formatedStr.replace(`style="${style}"`, /<video|<iframe/.test(str)?placeholerStyle: '');
  }
  ...
  return htmlContent
    ...
    .replace(/<iframe(.*?)src="(.*?)"(.*?)<\/iframe>/gi, function (str) {
      ...
      const { formatedStr } = getformatedStr(
        str,
        formatedAttributes,
       formatedAttributes.placeholderImage ? `style="background-image: ${formatedAttributes.placeholderImage}; background-size: 100% 100%;"` : ''
      );
