  ...
  return htmlContent
    .replace(
      ...
      function (str, p1, src, attrStr) {
        ...
        let alt = attributesArr.find((attr) => attr.startsWith("alt"));
        ...
        return `<span style="width: 100%;"><a class="gallery-item" href="${src}" target="_blank" rel="noopener" ${
          theme === "landscape" ? 'data-fancybox="gallery"' : ""
        }><span class="lazyload-wrap" data-content="${encodeURIComponent(
          str
        )}"><span class="placeholder"></span></span>${
          alt ? `<span class="caption">${alt}</span>` : ``
        }</a><noscript>${noScriptElement}</noscript></span>`;
      }
    )
    .replace(/<iframe(.*?)src="(.*?)"(.*?)<\/iframe>/gi, function (str) {
      ...
      return `<span style="width: 100%;"><span class="lazyload-wrap" data-content="${encodeURIComponent(
        str
