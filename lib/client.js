...
(function BlurHash() {
  ...
  function decodeBlurHash(o, r, e, n) {
    ...
    for (d = 1; d < i; d++) G = decode83(o, 4 + 2 * d, 6 + 2 * d), S[3 * d] = signSqr(~~(G / 361) - 9) * R, S[3 * d + 1] = signSqr(~~(G / 19) % 19 - 9) * R, S[3 * d + 2] = signSqr(G % 19 - 9) * R;
    var h = 4 * r,
      A = new Uint8ClampedArray(h * e);
    for (l = 0; l < e; l++) for (w = PI * l / e, f = 0; f < r; f++) {
      for (g = 0, u = 0, B = 0, C = PI * f / r, c = 0; c < a; c++) for (P = fastCos(w * c), d = 0; d < t; d++) I = fastCos(C * d) * P, p = 3 * (d + c * t), g += S[p] * I, u += S[p + 1] * I, B += S[p + 2] * I;
      q = 4 * f + l * h, A[q] = linearTosRGB(g), A[q + 1] = linearTosRGB(u), A[q + 2] = linearTosRGB(B), A[q + 3] = 255;
    }
    return A;
  }
...
(function () {
  ...
  var parseAltAttributes = function parseAltAttributes(string) {
    ...
    try {
      var placeholderImageRes = /\$placeholder=(\S+)=placeholder/gi.exec(string);
  ...
  var initPlaceholder = function initPlaceholder(element) {
    ...
    var contentStr = decodeURIComponent(element.dataset.content).replace("<img", "<div");
    ...
    var placeholderImage = resourceElement.dataset.placeholderimg || altAttributes.placeholderImage;
    if (placeholderImage && /blurhash:/.test(placeholderImage)) {
      var createCanvas = function createCanvas() {
        var canvas = document.createElement("canvas");
        var width = 32;
        var height = 32;
        var pixels = blurhash.decodeBlurHash(placeholderImage.replace("blurhash:", ""), width, height);
    ...
    if (inlineStyles) {
      appendStyles(element, inlineStyles);
      if (!!resourceElement.getAttribute("alt")) {
        var caption = element.parentElement.getElementsByClassName("caption")[0];
        if (caption) {
          appendStyles(caption, "margin-bottom: 1.25rem;");
        }
      }
    }
    element.dataset.content = element.dataset.content.replace(/%24placeholder%3D(\S+)%3Dplaceholder/gi, "").replace(/%24aspect-ratio%3D(\S+)%3Daspect-ratio/gi, "");
  };
